...
import { ChevronLeftIcon, ChevronRightIcon, ScrollIcon } from "@/component/icons";
...
import * as styles from "@/style/hunt-viewer.css";
import type { HuntViewerProps } from "@/type/components";

const ConfirmModal = lazy(() => {
	return import("@/component/confirm-modal");
});
...
export default (props: HuntViewerProps) => {
	...
	const filteredHistory = createMemo(() => {
		return props.history.filter((record) => {
			return detectHuntReportType(record.rawText) === "party";
		});
	});
	...
	const activeParty = createMemo(() => {
		return activeHunt()?.parsed ?? null;
	});

	const partyRankings = createMemo(() => {
		return dashboard.getPartyRankings(activeParty());
	});

	const pendingDelete = createMemo(() => {
		const id = pendingDeleteId();
		return id
			? props.history.find((record) => {
					return record.id === id;
				})
			: null;
	});
									...
									<div class={styles.viewerHeader}>
										...
										<div class={styles.viewerActions}>
											...
											<button
												...
												type="button"
												onClick={() => {
													return requestDelete(active().record.id);
												}}
																	...
																	<dl class={styles.partyMetricList}>
																		<For each={member.metrics}>
																			{(metric) => {
																				return (
																					<>
																						<dt class={styles.partyMetricLabel}>
																							{metric.label}
																						</dt>
																						<dd class={styles.partyMetricValue}>
																							{metric.value}
																						</dd>
																					</>
																				);
											...
											return (
												<section class={styles.partyRankingGrid} aria-label="Destaques da party">
													<section class={styles.partyRanking}>
														...
														<Show
															when={rankings().supplies}
															fallback={<p class={styles.panelEmpty}>Sem dados</p>}
														>
															{(top) => {
																return (
																	<>
